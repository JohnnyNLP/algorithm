...
- 핵심은 임시값 temp에 끊임없이 자기 자신과 sum(자릿수의 합)을 더하여 이 값을 list에서 제거하는 것이다.
- 위 문제에서 사용된 변수는 다음과 같이 세 개이다.
  - temp는 우리가 최종적으로 지우고자 하는 셀프 넘버가 아닌 수이다.
  - num은 자릿수의 합을 구하기 위해 임시적으로 temp값을 저장한 변수이다.
  - sum은 자릿수의 합을 저장한 변수이다.
- temp는 while문 안에서 계속해서 자기 자신과 sum을 더해나가면서 몸집을 키운다.
- 매 while 수행루프마다 temp 값과 일치하는 원소를 list에서 지운다.
- temp가 10000보다 커지는 시점에서 while 문을 종료한다.
...
### 개선된 접근 방식
 ...
 - 최초 시행에서는 재귀적 호출부의 i 범위를 10000으로 설정하였다. 1부터 10000까지의 모든 정수 값을 살펴본 것이다. 위 코드를 사용했을 때의 한계는 다음과 같았다.
   ...
   1. 이미 지워진 temp값에 대해서 동일한 작업을 반복한다.
...
- 나머지 정리를 이용한 방법으로, 수행 방식은 다음과 같다.
  1. num을 10으로 나눈 나머지는 num의 일의 자리 수이다.
  2. 이를 sum에 더해서 저장한다.
  3. num을 10으로 나눈 값을 자기 자신에게 재할당한다.
  4. 이때 num은 일의 자리 수가 지워진 값으로 저장된다.
...
- 그러나 이 경우에는 temp라는 구체적인 값에 대응하는 원소를 지워야 한다는 점과, 지워나갈 때마다 list의 인덱스 역시 변화한다는 점을 고려해야 한다.
